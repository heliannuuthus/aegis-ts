...
- **React Hook**: 提供 `useAuth` 用于 React 应用
- **Web 适配器**: `WebAuth` 简化浏览器端认证流程
...
### 方式三：useAuth（React Hook）

适用于 React 应用，需提供 `getAuth` 和 `getAuthorizeOptions`：
...
function App() {
  const {
    ...
    logout,
    getAccessToken,
  } = useAuth({ getAuth, getAuthorizeOptions });
...
- **类**: `Auth`, `WebAuth`, `AuthError`, `BrowserStorageAdapter`, `MemoryStorageAdapter`
- **Hook**: `useAuth`
- **类型**: `AuthConfig`, `AuthorizeOptions`, `CallbackResult`, `IDTokenClaims`, `UseAuthConfig`, `UseAuthReturn` 等
